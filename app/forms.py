...
class SetMacroForm(FlaskForm):
    ...
    protein = SelectField(
        "Protein", choices=PERCENT_CHOICES, validators=[DataRequired()], default=0.1
    )
...
class CopyMealForm(FlaskForm):
    ...
    meal_select = SelectField("Meal", choices=MEAL_OPTIONS, validators=[DataRequired()])
...
class AddToDiaryForm(FlaskForm):
    ...
    add = SubmitField("Add")
    meal = SelectField(
        label="Select Meal", choices=MEAL_CHOICES, validators=[DataRequired()]
    )
...
class DiaryDatePicker(FlaskForm):
    date = StringField(
        default=datetime.utcnow().strftime("%B %d, %Y"), validators=[DataRequired()]
    )
